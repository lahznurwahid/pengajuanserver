...
function getKepalaLabStatus(p: Pengajuan) {
  const persetujuanList = p.persetujuan || [];
  const wadekRejection = persetujuanList.find(
    (x: { status: string; user?: { role?: string } | null }) => x.user?.role?.toUpperCase() === 'WADEK' && x.status === 'DITOLAK'
  );
  if (wadekRejection) return 'DITOLAK';

  const kepalaLabDecision = persetujuanList.find(
    (x: { status: string; user?: { role?: string } | null }) => x.user?.role?.toUpperCase() === 'KEPALA_LAB' && (x.status === 'DISETUJUI' || x.status === 'DITOLAK')
  );
  ...
  const waitingCount = pengajuan.filter(p => getKepalaLabStatus(p) === 'MENUNGGU').length;
  const processedCount = pengajuan.filter(p => getKepalaLabStatus(p) === 'DIPROSES').length;
  const rejectedCount = pengajuan.filter(p => getKepalaLabStatus(p) === 'DITOLAK').length;
